...
struct FRawInputRegisteredDevice
{
	...
	FString SerialNumber;        // 컨트롤러 고유 식별을 위한 직렬 번호
	bool bIsWireless;            // 무선 연결 여부
	int32 ControllerIndex;       // 컨트롤러 인덱스 (플레이어 ID에 매핑 가능)
};
...
public:
	...
	HANDLE hDevice;
	FString SerialNumber;    // 새로 추가
	bool bIsWireless;        // 새로 추가
};
...
public:
	...
	virtual void Tick(float DeltaTime) override
	{
		...
		if (CurrentTime - LastDeviceCheckTime > DeviceCheckInterval)
		{
			//UE_LOG(LogRawInputWindows, Verbose, TEXT("Checking for connected devices at time %f (last check: %f)"),
			//	CurrentTime, LastDeviceCheckTime);
			LastDeviceCheckTime = CurrentTime; // 시간 업데이트를 이 위치에서 확실히 수행

			QueryConnectedDevices(); // 연결된 장치 다시 확인
		}
	}
	...
	FString GetDeviceSerialNumber(HANDLE DeviceHandle);
// my 
private :
	double LastDeviceCheckTime;

	const double DeviceCheckInterval = 2.0;
private:
	// my fun
	/*void ProcessDualSenseDpad(FRawWindowsDeviceEntry* DeviceEntry, uint8 dpadValue);*/
